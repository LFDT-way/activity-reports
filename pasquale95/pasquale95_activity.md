# GitHub activity report — `pasquale95`

_Limited to years 2024–2026._

Date range searched: **2015-01-01 → 2026-05-28**.

Compiled from **938** events spanning **2024-04-05 → 2026-05-20**, across **3** organizations and **10** repositories.

## Teams & repository access

Member of **1** team(s):

| Team | Role in team |
| --- | --- |
| `fabric-x-release-maintainers` | member |

Repository access granted through those teams (9 repos):

| Repository | Access | Granted by |
| --- | --- | --- |
| `fabric-x` | maintain | `fabric-x-release-maintainers` (maintain) |
| `fabric-x-committer` | maintain | `fabric-x-release-maintainers` (maintain) |
| `fabric-x-common` | maintain | `fabric-x-release-maintainers` (maintain) |
| `fabric-x-endorser` | maintain | `fabric-x-release-maintainers` (maintain) |
| `fabric-x-evm` | maintain | `fabric-x-release-maintainers` (maintain) |
| `fabric-x-orderer` | maintain | `fabric-x-release-maintainers` (maintain) |
| `fabric-x-rfcs` | maintain | `fabric-x-release-maintainers` (maintain) |
| `fabric-x-samples` | maintain | `fabric-x-release-maintainers` (maintain) |
| `fabric-x-sdk` | maintain | `fabric-x-release-maintainers` (maintain) |


## Overview

| Metric | Value |
| --- | --: |
| Total events | 938 |
| Pull requests opened | 50 |
| Pull request reviews | 31 |
| Issues opened | 20 |
| Pushes | 440 |
| Commits pushed | 97 |
| Comments | 54 |
| Releases | 37 |

## Activity by year

| Year | Events |
| --- | --: |
| 2024 | 14 |
| 2025 | 493 |
| 2026 | 431 |

## Event types

| Type | Count |
| --- | --: |
| PushEvent | 440 |
| CreateEvent | 136 |
| DeleteEvent | 104 |
| PullRequestEvent | 91 |
| IssuesEvent | 41 |
| ReleaseEvent | 37 |
| PullRequestReviewEvent | 31 |
| PullRequestReviewCommentEvent | 28 |
| IssueCommentEvent | 26 |
| WatchEvent | 3 |
| ForkEvent | 1 |

## Organizations

| Organization | Events | PRs opened | Reviews | Pushes |
| --- | --: | --: | --: | --: |
| [`LF-Decentralized-Trust-labs`](https://github.com/LF-Decentralized-Trust-labs) | 775 | 41 | 12 | 405 |
| [`hyperledger`](https://github.com/hyperledger) | 149 | 9 | 19 | 30 |
| [`hyperledger-labs`](https://github.com/hyperledger-labs) | 14 | 0 | 0 | 5 |

## Repositories

| Repository | Events | PRs opened | Reviews | Pushes |
| --- | --: | --: | --: | --: |
| [`LF-Decentralized-Trust-labs/fabric-x-ansible-collection`](https://github.com/LF-Decentralized-Trust-labs/fabric-x-ansible-collection) | 775 | 41 | 12 | 405 |
| [`hyperledger/fabric-x`](https://github.com/hyperledger/fabric-x) | 109 | 4 | 13 | 26 |
| [`hyperledger/fabric-x-orderer`](https://github.com/hyperledger/fabric-x-orderer) | 15 | 2 | 2 | 3 |
| [`hyperledger-labs/jsonld-vc-bbs-go`](https://github.com/hyperledger-labs/jsonld-vc-bbs-go) | 13 | 0 | 0 | 5 |
| [`hyperledger/fabric-x-committer`](https://github.com/hyperledger/fabric-x-committer) | 13 | 1 | 4 | 0 |
| [`hyperledger/fabric-x-common`](https://github.com/hyperledger/fabric-x-common) | 6 | 1 | 0 | 0 |
| [`hyperledger/fabric-ca`](https://github.com/hyperledger/fabric-ca) | 3 | 1 | 0 | 0 |
| [`hyperledger/fabric-x-samples`](https://github.com/hyperledger/fabric-x-samples) | 2 | 0 | 0 | 1 |
| [`hyperledger-labs/fabric-token-sdk`](https://github.com/hyperledger-labs/fabric-token-sdk) | 1 | 0 | 0 | 0 |
| [`hyperledger/fabric`](https://github.com/hyperledger/fabric) | 1 | 0 | 0 | 0 |

## Monthly rollup — one table per repository

Months as rows, event types as columns. One file per repository in [`pasquale95_rollup/`](pasquale95_rollup/):

| Repository | Events | Rollup |
| --- | --: | --- |
| [`LF-Decentralized-Trust-labs/fabric-x-ansible-collection`](https://github.com/LF-Decentralized-Trust-labs/fabric-x-ansible-collection) | 775 | [LF-Decentralized-Trust-labs_fabric-x-ansible-collection.md](pasquale95_rollup/LF-Decentralized-Trust-labs_fabric-x-ansible-collection.md) |
| [`hyperledger/fabric-x`](https://github.com/hyperledger/fabric-x) | 109 | [hyperledger_fabric-x.md](pasquale95_rollup/hyperledger_fabric-x.md) |
| [`hyperledger/fabric-x-orderer`](https://github.com/hyperledger/fabric-x-orderer) | 15 | [hyperledger_fabric-x-orderer.md](pasquale95_rollup/hyperledger_fabric-x-orderer.md) |
| [`hyperledger-labs/jsonld-vc-bbs-go`](https://github.com/hyperledger-labs/jsonld-vc-bbs-go) | 13 | [hyperledger-labs_jsonld-vc-bbs-go.md](pasquale95_rollup/hyperledger-labs_jsonld-vc-bbs-go.md) |
| [`hyperledger/fabric-x-committer`](https://github.com/hyperledger/fabric-x-committer) | 13 | [hyperledger_fabric-x-committer.md](pasquale95_rollup/hyperledger_fabric-x-committer.md) |
| [`hyperledger/fabric-x-common`](https://github.com/hyperledger/fabric-x-common) | 6 | [hyperledger_fabric-x-common.md](pasquale95_rollup/hyperledger_fabric-x-common.md) |
| [`hyperledger/fabric-ca`](https://github.com/hyperledger/fabric-ca) | 3 | [hyperledger_fabric-ca.md](pasquale95_rollup/hyperledger_fabric-ca.md) |
| [`hyperledger/fabric-x-samples`](https://github.com/hyperledger/fabric-x-samples) | 2 | [hyperledger_fabric-x-samples.md](pasquale95_rollup/hyperledger_fabric-x-samples.md) |
| [`hyperledger-labs/fabric-token-sdk`](https://github.com/hyperledger-labs/fabric-token-sdk) | 1 | [hyperledger-labs_fabric-token-sdk.md](pasquale95_rollup/hyperledger-labs_fabric-token-sdk.md) |
| [`hyperledger/fabric`](https://github.com/hyperledger/fabric) | 1 | [hyperledger_fabric.md](pasquale95_rollup/hyperledger_fabric.md) |

---

<sub>Generated by `gh-report` from `gh-activity` archive data.</sub>
